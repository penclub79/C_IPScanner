...
=======
>>>>>>> 7304b2aa6ed7718361ee3fc6c9e641aa9e0b9eb5
public:
	NetScanBase();			// 생성자
...
=======
	BOOL	StartScan( int iScanType, DWORD pdwThread );
	void	SocketBind();
	void	SetBindAddress( ULONG ulBindAddress );
	void	SetNotifyWindow( HWND hWnd, LONG msg );
	void	SetCloseMsgRecvWindow( HWND hWnd, LONG msg/* = WM_CLOSE*/ );
	BOOL	SendScanRequest(int iScanType);
	void	ThreadExit();
...
private:
	//////////////////////////////////////////////////////////// Variable
	HANDLE	m_hVisScanThread;	//Vision 스레드ID
	HANDLE	m_hMkScanThread;	//MarkIn 스레드ID
	HWND	m_hCloseMsgRecvWnd; //Close win핸들
	BOOL	m_bUserCancel; 
	DWORD	m_dwScanThreadID;
	LONG	m_lCloseMsg;
	SOCKET	m_hSockReceive;	//vision 소켓
	//SOCKET	m_hMkSockReceive;	//MarkIn 소켓
	HWND	m_hNotifyWnd;
	LONG	m_lNotifyMsg;
	ULONG	m_ulBindAddress;
	char*	m_pReceive_buffer;
	...
	//////////////////////////////////////////////////////////// Function
	void	WideCopyStringFromAnsi( WCHAR* pwszString, int iMaxBufferLen, char* pszString );
